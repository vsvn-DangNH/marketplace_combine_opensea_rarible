...
contract ExchangeCore is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function __ExchangeCore_init(uint256 _minimumMakerProtocolFee, uint256 _minimumTakerProtocolFee)
         initializer public
    {
        OwnableUpgradeable.__Ownable_init();
        ReentrancyGuardUpgradeable.__ReentrancyGuard_init();
        INVERSE_BASIS_POINT = 10000;
        minimumMakerProtocolFee = _minimumMakerProtocolFee;
        minimumTakerProtocolFee = _minimumTakerProtocolFee;
    }
    /**
    ...
    function atomicMatch(
        ...
        if (buy.maker == msg.sender) {
            console.log("Validating Buy Order Params");
            require(validateOrderParameters(buy));
        } else {
            console.log("Checking valid buy order");
        ...
        if (sell.maker == msg.sender) {
            console.log("Validating Sell Order Params");
            require(validateOrderParameters(sell));
        } else {
            console.log("Checking valid sell order");
            sellHash = requireValidOrder(sell, sellSig);
        }

        /* Must be matchable. */
        console.log("Checking if order can match");
        ...
        if (sell.howToCall == AuthenticatedProxy.HowToCall.Call) {
            (result, ) = sell.target.call(sell._calldata);
        }
